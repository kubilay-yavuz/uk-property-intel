...
Eight packages, each publishable as a standalone PyPI wheel, sharing a single
...
|-----------|-----------|--------------|--------|
| [`packages/scrapers`](packages/scrapers)           | `uk-property-scrapers`     | Pure-Python parsers for Zoopla / Rightmove / OnTheMarket + auction houses. Browser-agnostic: takes HTML, returns Pydantic `Listing`s. | Alpha |
| [`packages/apis`](packages/apis)                   | `uk-property-apis`         | Typed async clients for 15+ UK government / public APIs: EPC Open Data, HMLR Price Paid, Postcodes.io, data.police.uk, planning.data.gov.uk, Environment Agency Flood, Companies House, ONS Nomis / Census 2021, OSM Overpass, NaPTAN, Contracts Finder + Find a Tender, UKHO tides, BGS, and the auction-house registers. Retries, pagination, Pydantic models. | Alpha |
...
Expect ~1230 mocked tests across the 8 packages, all green (`uv run pytest --collect-only -q`).
...
`pyproject.toml` in each sibling). Once the 8 packages publish to PyPI, the
...
```bash
# Full mocked test suite (~1230 tests, <15s):
uv run pytest -q
